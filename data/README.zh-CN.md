...
| `species_taxonomy.csv`           | 100 类群对应的目 / 科 / 属（12 / 27 / 72）；源自测序公司谱系。 |
...
**本表取自流程的哪一阶段。** 测序公司以 DADA2 v1.18.0 生成 ASV，并用 BLASTn 比对 MitoFish 完成分类学注释。其下游
汇总表另加一道假阳性过滤：单样本内种水平丰度小于或等于 5 的读数被置零。**该过滤仅施于秋季交付数据，春季未施**，
故公司自带的汇总表在季节间不可比。本包各表取自该步骤之前的**未过滤** ASV 输出，因此两季节均保留单读数检出且数量
相当（春季 69 个、秋季 69 个单读数单元）。本包任何环节均未设置最小读数阈值或最小重复数标准。
...
秋季产卵洄游）在秋季交付中占读数的 17.1%，在春季交付中为 0.05%。重复求和可精确重现站点级表（在全部 2,600 个
类群 × 站点-季节单元中最大差异为 0）。
...
总磷、总氮与氨氮。野外指标使用 YSI ProQuatro 水质仪测定，营养盐于实验室测定。每个站点-季节各占一行，
因此水化学数据按**季节与站点**双键与群落数据合并，而非仅按站点合并。
